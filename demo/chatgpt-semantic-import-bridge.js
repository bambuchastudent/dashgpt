...
const MIN_SEMANTIC_SOURCE_VERSION = 5;
let installed = false;
let activeSemanticSession = "";
...
function answerHello(event, config, data) {
  ...
  activeSemanticSession = sessionKey(config);
...
export function installChatGptSemanticImportBridge() {
  ...
  window.addEventListener("message", event => {
    ...
    if (event.data.type !== "BATCH" || activeSemanticSession !== sessionKey(config)) return;
