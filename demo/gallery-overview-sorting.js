...
export const GALLERY_SORT_STATE_KEY = "dashgpt.demo.gallery-sort.v1";
export const GALLERY_SORT_MODES = Object.freeze(["time", "color", "tag"]);
...
const OVERVIEW_MAP_DESKTOP = Object.freeze({ width: 10, height: 10, gap: 2 });
const OVERVIEW_MAP_MOBILE = Object.freeze({ width: 6, height: 6, gap: 1 });
...
export function orderGalleryCards(items, mode = "time", events = []) {
  const safeMode = GALLERY_SORT_MODES.includes(mode) ? mode : "time";
  const decorated = (Array.isArray(items) ? items : []).map((result, index) => ({
    result,
    index,
    time: meaningfulCardTimestamp(result, events),
    hue: semanticHue(result),
...
export function planWholeBoardOverview({
  ...
  return {
    columns,
    rows,
    tileWidth: (safeWidth - heatGap * (columns - 1)) / columns,
    tileHeight: heatHeight,
    fits: false,
    score: Math.min(((safeWidth - heatGap * (columns - 1)) / columns) / heatWidth, 1),
    representation: "overflow",
    gap: heatGap
  };
}

function readSortMode(storage = globalThis.localStorage) {
  try {
    const parsed = JSON.parse(storage?.getItem?.(GALLERY_SORT_STATE_KEY) || "null");
    return GALLERY_SORT_MODES.includes(parsed?.sortMode) ? parsed.sortMode : "time";
  } catch {
    return "time";
  }
}

function writeSortMode(mode, storage = globalThis.localStorage) {
  const sortMode = GALLERY_SORT_MODES.includes(mode) ? mode : "time";
  storage?.setItem?.(GALLERY_SORT_STATE_KEY, JSON.stringify({ version: 1, sortMode }));
  return sortMode;
}
...
.gallery-region[data-f32-overview] .results-grid{grid-template-columns:repeat(var(--f32-overview-columns),minmax(0,1fr))!important;grid-auto-rows:var(--f32-overview-height);gap:var(--f32-overview-gap)!important;align-items:stretch}
...
.gallery-region[data-f32-overview="heatmap"] .result-card,.gallery-region[data-f32-overview="overflow"] .result-card{padding:0!important;border-radius:3px!important;background:linear-gradient(135deg,hsl(var(--semantic-hue),72%,50%),hsl(var(--semantic-hue-2),68%,40%))!important;box-shadow:none!important}
.gallery-region[data-f32-overview="heatmap"] .result-card::before,.gallery-region[data-f32-overview="overflow"] .result-card::before{height:100%;background:linear-gradient(90deg,hsla(var(--semantic-hue),92%,68%,.78),hsla(var(--semantic-hue-2),88%,62%,.38));box-shadow:none;opacity:.82}
...
@media(max-width:620px){.gallery-sort{width:100%;justify-content:space-between}.gallery-sort-label{display:none}.gallery-sort .button{flex:1;padding-inline:7px}.gallery-region[data-f32-overview="compact"] .result-card .category{font-size:7px!important}.gallery-region[data-f32-overview="compact"] .result-card .title{font-size:8px!important}}
`;
...
function overviewHeight(grid) {
  ...
  const visible = globalThis.innerHeight - Math.max(96, Math.min(rect.top, globalThis.innerHeight - 120)) - 20;
  return Math.max(320, visible, globalThis.innerHeight * 0.7);
}
