...
1. Register the public DashGPT GitHub App using the pre-filled configuration in `docs/github-storage-setup.md`.
2. Configure `GITHUB_APP_ID`, `GITHUB_APP_SLUG`, `GITHUB_APP_PRIVATE_KEY` and `GITHUB_SESSION_SECRET` on the production Worker without putting secrets in chat/repository/Vault.
3. Install the app on one disposable/private test repository and prove real pair → sync → idempotent re-sync → disconnect while local Vault survives.
...
- Production GitHub sync activation requires one GitHub App registration and Worker secret configuration.
