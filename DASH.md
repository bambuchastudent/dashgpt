...
**Active internal work:** Feature 6 Slice B — GitHub Vault sync activation.

- Feature 6 architecture PR #11 and Vault core PR #12 are merged into `develop`.
- GitHub sync implementation PR #13 is open from `feature/f6-github-sync`.
- PR #13 implements GitHub repository/folder StorageLocators, GitHub App pairing, signed HttpOnly pairing state, server-side installation tokens and Vault v1 synchronization.
- GitHub sync writes provider-neutral Vault files and creates one atomic Git commit per logical sync.
- Deterministic tests cover pairing, Vault object layout, local/remote merge, idempotent sync and conflict behavior.
...
- Remaining Slice B external gate: register/configure the public GitHub App + Worker secrets, then run one real private-repository smoke test.
...
- GitHub adapter security architecture recorded in ADR 0004.

## NEXT

1. Make the final PR #13 revision green and merge the GitHub sync implementation.
2. Register the public DashGPT GitHub App using the pre-filled configuration in `docs/github-storage-setup.md`.
3. Configure `GITHUB_APP_ID`, `GITHUB_APP_SLUG`, `GITHUB_APP_PRIVATE_KEY` and `GITHUB_SESSION_SECRET` on the production Worker without putting secrets in chat/repository/Vault.
4. Install the app on one disposable/private test repository and prove real pair → sync → idempotent re-sync → disconnect while local Vault survives.
5. Mark Slice B fully activated only after that real smoke test.
6. Start Slice C in a new PR: Google Drive StorageLocator + authorization + adapter contract reuse.
7. Then implement explicit inspectable DashGPT Profile / preference-promotion boundary in Slice D.
8. Refactor compatible DashGPT instance storage onto the same abstraction and add provider switch/mirror UX in Slice E.
9. Separately resume the Feature 4 external OpenAI plugin submission flow when desired.

## BLOCKERS / EXTERNAL HINGES

- GitHub sync code has no remaining architectural blocker; production activation requires one GitHub App registration and Worker secret configuration.
