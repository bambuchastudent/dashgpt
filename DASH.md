...
- User action required now: **create the DashGPT plugin draft in OpenAI Platform while identity review continues**.
...
1. Open OpenAI Platform → Plugins and create a **DashGPT** plugin draft.
2. Choose **With MCP** and **Universal** MCP URL.
3. Use the production MCP endpoint: `https://dashgpt.dimkashir.workers.dev/mcp`.
4. Complete domain verification when the portal provides the challenge token, then Scan Tools.
5. Wait for Individual publisher identity review to finish; once verified, select it as Developer Identity.
...
- OpenAI Platform Individual publisher verification is currently in review; final submission cannot use the identity until verification completes.
