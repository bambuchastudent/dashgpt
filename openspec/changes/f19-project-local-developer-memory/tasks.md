...
- [x] 1.5 Revalidate the expanded Project State visualization scope before modifying renderer production code.
...
- [x] 3.1 Extend the fixture with verified `workstream`, `stage`, `stageLabel`, `priority`, `progress` and product-spine metadata.
- [x] 3.2 Replace Project Map as the default tab with Project State.
- [x] 3.3 Add a compact product spine showing how DashGPT capture, cards, semantic organization, continuation and developer memory compose.
- [x] 3.4 Add a prominent Now surface where active cards expose user-visible outcome and progress/evidence without opening details.
- [x] 3.5 Add Shipped / Active / Next project-state summary derived from canonical cards.
- [x] 3.6 Group project cards by human workstream while keeping semantic hue as a secondary cue.
- [x] 3.7 Keep card details, Timeline, Results and Sessions using the same canonical IDs.
...
- [x] 4.3 Update browser coverage for default Project State, product spine, active work/outcome/progress, stage distinctions and workstreams.
- [x] 4.4 Verify the revised Project State at desktop and 390px mobile widths with no horizontal overflow.

## 5. Final verification

- [x] 5.1 Run strict OpenSpec validation after the scope revision.
- [x] 5.2 Run the repository canonical full verification (`npm run check` plus Playwright/browser gate through CI) on the Project State implementation head.
- [x] 5.3 Verify the production preview deployment status. Cloudflare reports a successful deployment of the Project State implementation head; direct interactive remote-preview inspection remains unavailable in this execution environment, so visual behavior is verified through the desktop/mobile browser gate.
